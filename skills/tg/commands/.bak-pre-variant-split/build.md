---
name: tg:guide
description: Generate interactive HTML technical guides with quizzes (parallel sub-agents)
allowed-tools:
  - Read
  - Write
  - Bash
...
interactive HTML technical guide for one chapter, with pre/post quizzes, key points,
...
<execution_context>
@~/.claude/tg/prompts/guide-gen.md
@~/.claude/tg/templates/techguide-base.html
...
2. **Read outline.json**, **config.json**, and the **techguide-base.html template**
...
4. **Spawn parallel guide sub-agents** via Agent tool:
   ...
   ```
   You are an interactive technical guide generator for the Techguide pipeline.

   TASK: Create an interactive HTML technical guide for Chapter {N}: "{title}"

   Read these files for context:
   - Chapter content: {topic_dir}/chapters/chapter-{nn}.md
   - HTML template: ~/.claude/tg/templates/techguide-base.html
   - Quiz schema: ~/.claude/tg/templates/quiz-schema.json
...
9. **Report**: "Generated {N} interactive technical guides in guides/"

10. **Auto-invoke `/tg:slides`** — IMMEDIATELY invoke via Skill tool.
</process>
