...
1. **Find active topic**:
   ...
   - If no topics at all, tell user to run `/sws:start` first

2. **Read config.json**:
   - Read `output/{slug}/config.json`
   - Extract: topic, depth, examples, analogies, visual_emphasis

3. **Load scoping prompt** from `~/.claude/sws/prompts/scoping.md`
...
10. **Verify outline**:
    ```bash
    node "$HOME/.claude/sws/bin/sws-tools.cjs" outline-chapters output/{slug}
    ```

11. **Auto-invoke `/sws:research`** — IMMEDIATELY invoke via Skill tool. Do NOT just tell the user to run it.
</process>
