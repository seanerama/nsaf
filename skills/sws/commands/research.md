...
<execution_context>
@~/.claude/sws/prompts/research.md
@~/.claude/sws/references/pipeline-stages.md
@~/.claude/sws/references/mcp-setup.md
</execution_context>
...
1. **Find active topic**:
   ...
   node "$HOME/.claude/sws/bin/sws-tools.cjs" active-topic
   ```
   - If no topic found or pipeline_stage is not "scoped:true", tell user to run `/sws:start` or `/sws:scope` first
...
4. **Spawn parallel research sub-agents** via Agent tool:
   ...
   ```
   You are a research agent for the StudyWS learning pipeline.
...
9. **Auto-invoke `/sws:write`** — IMMEDIATELY invoke via Skill tool. Do NOT just tell the user to run it.
</process>
