...
def _load_studyws_config(project_dir):
    """Read studyws-config.json from the project root. Returns {} on miss."""
    import json as _json
    path = os.path.join(project_dir, "studyws-config.json")
    try:
        with open(path) as f:
            return _json.load(f)
    except (OSError, ValueError):
...
def _extract_textbook_intro(textbook_path, max_len=320):
    """Return the first non-empty paragraph after the H1 in a textbook.md,
    flattened to a single line and trimmed. Returns '' if unavailable."""
    import re
    try:
    ...
    body = after_h1[1] if len(after_h1) == 2 else text
...
def _promote_study(rest):
    ...
    sws_config = _load_studyws_config(project_dir)
    title = (sws_config.get("topic") or "").strip() or (
        (project.get("slug") or web_slug).replace("sws-", "").replace("-", " ").title()
    )
    desc = _extract_textbook_intro(textbook_src) if os.path.isfile(textbook_src) else ""
    if not desc:
        level = (sws_config.get("level") or "intermediate").strip()
        desc = f"{level.title()}-level study guide on {title}, covering {len(html_files)} interactive chapters."
    ...
    # Step 5: bun verify build (with one stale-cache retry if the per-slug page is missing)
    def _rollback():
    ...
    dist_index = os.path.join(repo, "dist", "study-guides", web_slug, "index.html")
    try:
        result = _bun_build()
        if result.returncode != 0:
        ...
        if not os.path.isfile(dist_index):
            # Astro stale-cache for new collections — clear and rebuild once
            shutil.rmtree(os.path.join(repo, ".astro"), ignore_errors=True)
            ...
            if result.returncode != 0 or not os.path.isfile(dist_index):
                ...
                _rollback()
                lines.append(
                    f"4. ❌ Build produced no `dist/study-guides/{web_slug}/index.html` "
                    f"even after clearing `.astro`/`dist` — changes reverted.\n```\n"
                    + "\n".join(tail) + "\n```"
                )
                return "\n".join(lines)
            lines.append(f"4. ✅ `bun run build` passed (after stale-cache clear)")
        else:
            lines.append(f"4. ✅ `bun run build` passed — `{web_slug}/index.html` rendered")
    except subprocess.TimeoutExpired:
        _rollback()
        lines.append("4. ❌ `bun run build` timed out after 5 minutes — changes reverted.")
        return "\n".join(lines)

    # Step 6: commit + push (rebase-and-retry once if remote drifted during build)
    try:
        _run_git(repo, "add",
